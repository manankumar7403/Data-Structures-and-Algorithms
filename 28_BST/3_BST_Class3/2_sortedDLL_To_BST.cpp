...
Node* sortedDLL_IntoBST(Node* &head, int n) // head and number of nodes
{
    // Base Case
    if(n <= 0 || head == NULL) return NULL;                     // nodes hi khatam hai ya head null hai

    // Solving for the left subtree
    Node* leftSubtree = sortedDLL_IntoBST(head, n-1-n/2);
    
    Node* root = head;
    root->left = leftSubtree;

    head = head->right;                          // important line

    // right part solve karna hai
    root->right = sortedDLL_IntoBST(head, n/2);
    return root;
}
// ✅ RECURSION HI SAB KUCH HAI
...
int main()
{
    ...
    Node* root1 = NULL;
    root1 = sortedDLL_IntoBST(head, 9);
    cout << "Doing level order traversal of root1:" << endl;
