...
void rightViewTree(Node* root, vector<int> &ans, int currLevel)
{
    // base case
    if(root == NULL)
    {
    ...
    // right
    rightViewTree(root->right, ans, currLevel+1);
    // left
    rightViewTree(root->left, ans, currLevel+1);
}
...
int main()
{
    ...
    vector<int> ans;
    int currLevel = 0;
    rightViewTree(root, ans, currLevel);
