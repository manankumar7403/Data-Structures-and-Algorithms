...
int mincostTickets_helper(vector<int> &days, vector<int> &costs, int i) { // i will traverse days array
    // Base condition
    if (i >= days.size())
        return 0;
    ...
    int j = i;                        // taking j=i, we can't call function for i+1, because when we take a 7 days pass, we need to specify the end day of the pass.
...
{
    int n;
    cout << "Enter the length of the days array: " << endl;
    cin >> n;
    vector<int>days(n);
    cout << "Enter the days on which you wish to travel: " << endl;
    for(int i=0; i<n; i++)
    {
        cin >> days[i];
    }
    ...
    cout << "Minimum dollars needed for travel: " << mincostTickets(days, costs);
    cout << endl;
    return 0;
}
...
// This recursive solution doesn't run (Gives TLE Error), will learn DP and revisit this problem.