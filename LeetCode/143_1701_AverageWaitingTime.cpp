...
The customer waits till the chef finishes preparing his order. The chef does not prepare food for more than one customer
at a time. The chef prepares food for customers in the order they were given in the input.
...
2) The second customer arrives at time 2, the chef takes his order and starts preparing it at time 3, and finishes at time
8, so the waiting time of the second customer is 8 - 2 = 6.
3) The third customer arrives at time 4, the chef takes his order and starts preparing it at time 8, and finishes at time
11, so the waiting time of the third customer is 11 - 4 = 7.
...
{
    long busyUptil = 0, n = customers.size(), waitingTime = 0;
    for (int i = 0; i < n; i++)
    {
        long arrivalTime = customers[i][0], timeTaken = customers[i][1];
        waitingTime = waitingTime + max(arrivalTime, busyUptil) + timeTaken - arrivalTime;
        busyUptil = max(arrivalTime, busyUptil) + timeTaken;
    }
