...
public:
    vector<int> arr;
    unordered_map<int, unordered_set<int>> Map; // to store multiple indices for each value
    RandomizedCollection() {
        
    }
    
    bool insert(int val) {
        bool notPresent = (Map.find(val) == Map.end());
        int index = arr.size();
        arr.push_back(val);
        Map[val].insert(index);  // to store a value corresponding to multiple indices
        return notPresent;
    }
    ...
    bool remove(int val) {
        ...
        {
            return false; // If the value is not found or no indices are stored, return false
        }
        int index = *(Map[val].begin()); //Retrieve the first index from the set, gets iterator of first idx(begin), then dereferences
        Map[val].erase(index); // removes the index from the set of indices
        
        if(Map[val].empty())
        {
            Map.erase(val);  // if set is empty, means no more indices present, so val can be erased from the Map
        }

        if(index != arr.size()-1)
        {
            int last = arr.back(); // last holds the last ele of the array
            arr[index] = last;
            Map[last].erase(arr.size()-1);  // remove the corresponding last element index
            Map[last].insert(index); // put the new index of the previously last element at the new location
        }
        arr.pop_back();
        return true;
    }
...
int main()
{
    ...
    cout << rc.insert(1) << endl;
    cout << rc.insert(2) << endl;
    cout << rc.getRandom() << endl;
    cout << rc.remove(1) << endl;
    cout << rc.getRandom() << endl;
    return 0;
}
